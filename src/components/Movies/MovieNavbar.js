...
function MovieNavbar() {
    ...
    function movieToggler(){
      ...
      function searchMe(event){
        ...
        let getAllMovies = AllMovies
        for( let i=0;i<getAllMovies.length;i++){
          allMovieTitles[i] = AllMovies[i]["Title"]
        }
    
        console.log('AMT',allMovieTitles);
    
        const matchingItems = allMovieTitles.filter(item => item.toLowerCase().includes(lowerVal));
          if (matchingItems.length === 0) {
            ...
            matchingItems.forEach((item,i) => {
              console.log(item);
              window.location.href = `/moviemania/${item}`
            });
           }       
          }
      }
  ...
  return (
    ...
    <div className="movie-search">
          ...
          <h3 id='noMatches' style={{color:'red', fontWeight:'500' ,marginTop:'10px', display:'none'}}>No results found...</h3>
    </div>
    </>
  )
}
